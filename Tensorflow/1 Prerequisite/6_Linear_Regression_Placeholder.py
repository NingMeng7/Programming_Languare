...
data, n_samples = utils.read_birth_life_data(DATA_FILE)	# utils is a module written in advance

# Step 2: create placeholders for X (birth rate) and Y (life expectancy)
X = tf.placeholder(tf.float32, name='X')  # If the shape is not specified, you can feed a tensor of any shape.
...
while you can think of a Tensor as representing the data being fed into your model and the intermediate 
representations of that data as it passes through your model.
'''

# Step 5: use the squared error as the loss function
# you can use either mean squared error or Huber loss
loss = tf.square(Y - Y_predicted, name='loss')	# tensor
